...
export default function Signin() {
  ...
  const getCurrentUserEmailAndUserId = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/auth/get-user`, {
        method: "POST",
        headers: {
  ...
  const handleSignIn = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/auth/initiate-auth`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
